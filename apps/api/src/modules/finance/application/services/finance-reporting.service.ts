...
export class FinanceReportingService {
  ...
  async getDashboardSummary(
    ...
    if (hasFilters) {
      const report = await this.getFlexibleReport(filters);
      return {
        ...
        net_expenses: report.net_expenses,
        profit_loss: report.profit_loss,
        total_refunds: report.total_refunds,
        authorized_credit: report.authorized_credit,
      };
    }

    const [[revenueRow], [collectedRow], [expenseRow], [adjustmentRow], [refundRow], [creditRow]] =
      await Promise.all([
        this.db
          .select({
        ...
        this.db
          ...
          .innerJoin(schema.payments, eq(schema.paymentAllocations.payment_id, schema.payments.id))
          .innerJoin(
    ...
    const profitLoss = toTwoDecimals(totalCollected - netExpenses - totalRefunds);
  ...
  async getRegistrationFinanceSummary(registrationId: string) {
    ...
    const directAdjustments = toTwoDecimals(Number(directAdjustmentRow?.total ?? 0));
    ...
    if (registrationRow?.traveller_id) {
      const [allocExpenseRow] = await this.db
        ...
        .where(
          and(
            eq(schema.expenseAllocations.traveller_id, registrationRow.traveller_id),
            eq(schema.expenseAllocations.is_deleted, false)
          )
        );
      allocatedGroupExpenses = toTwoDecimals(Number(allocExpenseRow?.total ?? 0));
    }
    ...
    const [creditRow] = await this.db
      ...
      .innerJoin(
        schema.financeExceptionStatuses,
        eq(schema.financeExceptions.finance_exception_status_id, schema.financeExceptionStatuses.id)
      )
      .where(
        and(
    ...
    const totalCost = toTwoDecimals(directExpenses + allocatedGroupExpenses + directAdjustments);
  ...
  async getTravelGroupFinanceSummary(travelGroupId: string) {
    ...
    if (registrationIds.length > 0) {
      ...
      groupRevenue = toTwoDecimals(invoiceRows.reduce((s, r) => s + Number(r.total_amount), 0));
    ...
    const groupAdjustments = toTwoDecimals(Number(groupAdjustmentRow?.total ?? 0));
  ...
  async getPackageVersionFinanceSummary(packageVersionId: string) {
    ...
    if (groupIds.length > 0) {
      ...
      if (registrationIds.length > 0) {
        ...
        totalRevenue = toTwoDecimals(invoiceRows.reduce((s, r) => s + Number(r.total_amount), 0));
  ...
  async getFlexibleReport(filters: {
    ...
    if (registration_id) {
      ...
      travel_round_id ||
      travel_round_from !== undefined ||
      travel_round_to !== undefined
    ) {
      const roundConditions = [eq(schema.travelRounds.is_deleted, false)];
      if (travel_round_id) roundConditions.push(eq(schema.travelRounds.id, travel_round_id));
      if (travel_round_from !== undefined) {
        roundConditions.push(sql`${schema.travelRounds.round_number} >= ${travel_round_from}`);
      }
      if (travel_round_to !== undefined) {
        roundConditions.push(sql`${schema.travelRounds.round_number} <= ${travel_round_to}`);
      }
      const roundRows = await this.db
        .select({ id: schema.travelRounds.id })
      ...
      if (roundIds.length === 0) {
        ...
        const registrationRows = await this.db
          ...
          .where(
            and(
              inArray(schema.registrations.travel_round_id, roundIds),
              eq(schema.registrations.is_deleted, false)
            )
          );
        ...
        const groupRows = await this.db
          ...
          .where(
            and(
              inArray(schema.groupMemberships.registration_id, registrationIds),
              eq(schema.groupMemberships.is_deleted, false)
            )
          );
        groupIdsForScope = [...new Set(groupRows.map((row) => row.travel_group_id))];
      }
    ...
    if (registrationIds !== null) {
      if (registrationIds.length === 0) {
        ...
        invoiceFilters.push(inArray(schema.invoices.registration_id, registrationIds));
      }
    }
    if (date_from)
      invoiceFilters.push(sql`${schema.invoices.invoice_date} >= ${new Date(date_from)}`);
    if (date_to) invoiceFilters.push(sql`${schema.invoices.invoice_date} <= ${new Date(date_to)}`);
    ...
    const totalRevenue = toTwoDecimals(invoiceRows.reduce((s, r) => s + Number(r.total_amount), 0));
    const invoiceIds = invoiceRows.map((r) => r.id);
      ...
      if (groupIdsForScope && groupIdsForScope.length > 0) {
        expenseFilters.push(
          or(
            ...
            inArray(schema.expenses.travel_group_id, groupIdsForScope)
          )!
        );
      } else {
        expenseFilters.push(inArray(schema.expenses.registration_id, registrationIds));
      }
    }
    if (expense_category_id)
      expenseFilters.push(eq(schema.expenses.expense_category_id, expense_category_id));
    if (expense_source_id)
      expenseFilters.push(eq(schema.expenses.expense_source_id, expense_source_id));
    if (date_from)
      expenseFilters.push(sql`${schema.expenses.expense_date} >= ${new Date(date_from)}`);
    if (date_to) expenseFilters.push(sql`${schema.expenses.expense_date} <= ${new Date(date_to)}`);
      ...
      if (groupIdsForScope && groupIdsForScope.length > 0) {
        adjustmentFilters.push(
          or(
            ...
            inArray(schema.expenseAdjustments.travel_group_id, groupIdsForScope)
          )!
        );
      } else {
        adjustmentFilters.push(inArray(schema.expenseAdjustments.registration_id, registrationIds));
      }
    }
    if (date_from)
      adjustmentFilters.push(
    ...
    if (registrationIds !== null) {
      ...
      if (invoiceIds.length > 0) {
        ...
        const paymentIds = [...new Set(paymentRegRows.map((r) => r.payment_id))];
        if (paymentIds.length > 0) {
          ...
          refundFilters.push(inArray(schema.refunds.registration_id, registrationIds));
        }
      } else {
        refundFilters.push(inArray(schema.refunds.registration_id, registrationIds));
      }
    }
    if (date_from) refundFilters.push(sql`${schema.refunds.refund_date} >= ${new Date(date_from)}`);
    if (date_to) refundFilters.push(sql`${schema.refunds.refund_date} <= ${new Date(date_to)}`);
    ...
    if (registrationIds !== null) {
      creditFilters.push(inArray(schema.financeExceptions.registration_id, registrationIds));
    }
    const [creditRow] = await this.db
      .select({
        total: sql<number>`coalesce(sum(${schema.financeExceptions.authorized_amount}), 0)`,
    ...
    if (date_from)
      unallocFilters.push(sql`${schema.payments.payment_date} >= ${new Date(date_from)}`);
    if (date_to) unallocFilters.push(sql`${schema.payments.payment_date} <= ${new Date(date_to)}`);
    ...
    const unallocated = toTwoDecimals(
      Number(unallocRow?.total_amount ?? 0) - Number(unallocRow?.allocated ?? 0)
    );
