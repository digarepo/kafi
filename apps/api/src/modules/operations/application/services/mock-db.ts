/**
 * Minimal chainable Drizzle-style mock for operations service unit tests.
 *
 * Every query builder method returns the same mock instance so that
 * `await db.select().from().where()` resolves the next queued value.
 * Inserts and updates record their payload for easy assertion.
 */
export class MockDb {
  queue: unknown[] = [];
  calls: string[] = [];
