...
export class RegistrationsService {
  ...
  async listRegistrations(dto: RegistrationFiltersDto) {
    const { page, page_size, search, traveller_id, package_version_id, status_id } = dto;
    const filters = [eq(schema.registrations.is_deleted, false)];
    if (traveller_id) filters.push(eq(schema.registrations.traveller_id, traveller_id));
    if (package_version_id)
      filters.push(eq(schema.registrations.package_version_id, package_version_id));
    if (status_id) filters.push(eq(schema.registrations.registration_status_id, status_id));
    if (search) {
      filters.push(
        or(
    ...
    const [rows, count] = await Promise.all([
      this.db
        ...
        .innerJoin(
          ...
          eq(schema.packageVersions.package_template_id, schema.packageTemplates.id)
        )
        .leftJoin(
          schema.packageVersionStatuses,
          eq(schema.packageVersions.package_version_status_id, schema.packageVersionStatuses.id)
        )
        .where(and(...filters))
  ...
  async getRegistration(id: string) {
    const [row] = await this.db
      ...
      .leftJoin(schema.countries, eq(schema.travellers.country_id, schema.countries.id))
      .leftJoin(
        schema.registrationStatuses,
        eq(schema.registrations.registration_status_id, schema.registrationStatuses.id)
      )
      ...
      .leftJoin(schema.currencies, eq(schema.packageVersions.currency_id, schema.currencies.id))
      .leftJoin(schema.seasons, eq(schema.packageVersions.season_id, schema.seasons.id))
      .leftJoin(
      ...
      .leftJoin(
        ...
        eq(schema.travellerContacts.contact_person_id, schema.contactPersons.id)
      )
  ...
  async createRegistration(dto: CreateRegistrationDto, actorId: string) {
    const [traveller] = await this.db
      ...
      .where(
        and(eq(schema.travellers.id, dto.traveller_id), eq(schema.travellers.is_deleted, false))
      )
      .limit(1);
  ...
  async updateRegistration(id: string, dto: UpdateRegistrationDto, actorId: string) {
  ...
  async startProcessing(id: string, actorId: string) {
    const existing = await this.getRegistration(id);
    if (existing.status === "CANCELLED") {
      throw new ConflictException("Cannot start processing a cancelled registration");
    }
    if (existing.status === "PROCESSING") {
      return existing;
    }
    if (existing.status !== "DRAFT") {
      throw new ConflictException(`Cannot start processing from status ${existing.status}`);
    }

    const isComplete = await this.readiness.isRegistrationComplete(id);
    if (!isComplete) {
      throw new ConflictException("Registration intake conditions are not satisfied");
    }
  ...
  async confirmReadyForTravel(id: string, actorId: string) {
    ...
    if (existing.status !== "PROCESSING") {
      throw new ConflictException(`Cannot confirm ready from status ${existing.status}`);
    }

    const isReady = await this.readiness.isReadyForTravel(id);
    if (!isReady) {
      throw new ConflictException("Registration readiness conditions are not satisfied");
    }
  ...
  async cancelRegistration(id: string, dto: CancelRegistrationDto, actorId: string) {
    const existing = await this.getRegistration(id);
    if (existing.status === "COMPLETED" || existing.status === "CANCELLED") {
      throw new ConflictException(`Cannot cancel a ${existing.status.toLowerCase()} registration`);
    }
    ...
    if (activeMembership) {
      throw new ConflictException("Cannot cancel a registration with an active group membership");
    }
    ...
    if (CANCELLATION_POLICY.hotelBookingBlocksCancellation) {
      const [hotelStay] = await this.db
        ...
        .innerJoin(
          ...
          eq(schema.groupMemberships.travel_group_id, schema.groupHotelStays.travel_group_id)
        )
        .where(
          and(
            eq(schema.groupMemberships.registration_id, id),
      ...
      if (hotelStay) {
        throw new ConflictException("Cannot cancel a registration with a confirmed hotel booking");
      }
    }
  ...
  private async computeCancellationFinancials(registrationId: string) {
    ...
    const summary = await this.financeReporting.getRegistrationFinanceSummary(registrationId);
  ...
  private async getRegistrationStatus(code: string) {
    const [row] = await this.db
      ...
      .limit(1);
    if (!row) throw new NotFoundException(`Registration status ${code} not found`);
    return row;
  }
    ...
    return {
      ...
      traveller: row.travellers
        ? {
            ...
            full_name: `${row.travellers.first_name} ${row.travellers.last_name}`.trim(),
    ...
    return {
      ...
      traveller: row.travellers
        ? {
            ...
            country: row.countries ? { id: row.countries.id, name: row.countries.name } : null,
      ...
      primary_contact: row.contact_persons
        ? {
            ...
            name: `${row.contact_persons.first_name} ${row.contact_persons.last_name}`.trim(),
            phone_number: row.contact_persons.phone_number,
          }
        : null,
      season: row.seasons ? { id: row.seasons.id, name: row.seasons.name } : null,
    };
  }
}
