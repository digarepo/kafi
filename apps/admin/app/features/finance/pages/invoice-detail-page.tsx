...
import { api, type Invoice, type UpdateInvoiceInput } from "../../../lib/api.js";
...
export function InvoiceDetailPage({ id }: InvoiceDetailPageProps) {
  ...
  async function handleArchive() {
    ...
    if (
      !(await confirm({
        ...
        description: "The invoice will be removed from active records and can be restored later.",
  ...
  if (!invoice) return <p className="text-destructive">{error ?? "Invoice not found"}</p>;
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="font-medium">{Number(invoice.discount_amount).toFixed(2)}</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-sm text-muted-foreground">Outstanding balance (ETB)</p>
            <p className="font-medium">{invoice.outstanding_balance.toFixed(2)}</p>
          </div>
        </CardContent>
      </Card>
